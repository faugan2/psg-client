...
const Challenge=({tournament,btnText,btnClick,detail,share_challenge,closed,started})=>{
    ...
    return(<div className="lobby__tournament" style={{}}>
			
            {(tournament.url!=null && tournament.url!="") ? 
				
				<img src={tournament.url} className="tournament_image" alt="" style={{marginBottom:"0.2rem"}}/>
				:<div />
			}
            
    
    <div className="tournament_row_two" style={{paddingTop:"0.5rem"}} >
        <div>
            {tournament.multiple==true? <p>Multiple</p>:<p>{ch_date}</p>}
            <p>Date</p>
        </div>
        ...
        <div>
            ...
            {
               (closed==true || started==true) && <p>Closed</p> 
            }
            {
                (closed==false &&  started==false) && <p>Open</p>
            }
            <p>State</p>
        </div>
    </div>
...

{
    ...
    <div className="" style={{
        ...
        }
        {
			...
			onClick={btnClick} className={`${new_text_btn}`} data-joined={joined}>
                {new_text_btn} 
            </button>
        }
    </div>
