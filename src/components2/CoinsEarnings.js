import "../styles/coins_earnings.scss";
...
const CoinsEarnings=({transactions,tournaments})=>{
    ...
    useEffect(()=>{
        ...
        const dates=res2.map((item)=>{
            const date=moment(item.data?.seconds*1000).format("ll");
        ...
        for(var i=0; i<dates.length; i++){
            ...
            const el=[];
            res2.map((item)=>{
                const item_date=moment(item.data?.seconds*1000).format("ll");
                if(date==item_date){
                    el.push(item);
                }
            })
